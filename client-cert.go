...
// Code below is taken from /usr/share/go/src/pkg/crypto/x509/x509.go in the 
// libgo0:amd64:4.7.2-5 package in debian wheezy.
// The function CreateCertificate has been adapted to create client certificates.
...
// CreateClientCertificate creates a new *client* certificate based on a template. The
// following members of template are used: SerialNumber, Subject, NotBefore,
// NotAfter, KeyUsage, BasicConstraintsValid, IsCA, MaxPathLen, SubjectKeyId,
// DNSNames, PermittedDNSDomainsCritical, PermittedDNSDomains.
// And ExtKeyUsage to specify client certificates.
//
...
// for priv).
func CreateClientCertificate(rand io.Reader, template, parent *Certificate, pub interface{}, priv interface{}) (cert []byte, err error) {
        ...
        extensions, err := mybuildExtensions(template)
        if err != nil {
                return
        }
...
func mybuildExtensions(template *Certificate) (ret []pkix.Extension, err error) {
	...
	if len(template.ExtKeyUsage) > 0 {
		ret[n].Id = []int{2, 5, 29, 37} //,  {2, 5, 29, 37, 0???}  // oid for ExtKeyUsage
		
		usages := make([]asn1.ObjectIdentifier, len(template.ExtKeyUsage))
		for i, usage := range template.ExtKeyUsage {
			switch usage {
...
/////////////////////////////////////////////////////////////////////////////
// Copied from x509.go because I cannot reference local identifiers...
// Nasty syntax rule of Go.
